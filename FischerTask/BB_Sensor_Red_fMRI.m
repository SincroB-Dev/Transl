...
%for block = 1:size(TrialList,2)
    ...
    for trial = 1:length(TrialList{2})
       ...
       [Posbasket_upcorner,Posbasket_downcorner] = posbasket(1100*P,700*P,2.5*TrialList{2}(3,1)*P*3,1.4);  
       ...
       [Posbasket_upcorner,Posbasket_downcorner] = posbasket(2000*P,700*P,2.5*TrialList{2}(3,2)*P*3,1.4);  
       Screen('PutImage', wPtr, basket, round([Posbasket_upcorner(1) Posbasket_upcorner(2) Posbasket_downcorner(1) Posbasket_downcorner(2)]*P));
        ...
        %Fishes Lake A
        proportion_fish=TrialList{2}(TrialOrder(trial),3);
        ...
        %Fishes Lake B
        proportion_fish=TrialList{2}(TrialOrder(trial),4);
       ...
       if trial == 1
            ...
            OutputMatrix(1,1) = 2; % block order
            OutputMatrix(1,2:6) = TrialList{2}(TrialOrder(trial),1:5); % trial characterics
            OutputMatrix(1,7)=t1;
            ...
            OutputMatrix(currentline,1) = 2; % block order
...
%DrawFormattedText(wBr,'Le jeu est maintenant terminé. Merci!','center', LowDisp, black);
